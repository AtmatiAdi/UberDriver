...
void DPWMMIN_Block(uint16_t PWM_Value){
	...
	if(Angle != OldAngle){
		...
		SetPulse_CH((((uint32_t)DPWMMIN_48_minus7_5[tmpAngle>>3])*(PWM_Value+1))>>8);// Rapid divison by 256

		tmpAngle += 128;
		if(tmpAngle >= 384) tmpAngle -= 384;
		SetPulse_AH((((uint32_t)DPWMMIN_48_minus7_5[tmpAngle>>3])*(PWM_Value+1))>>8);// Rapid divison by 256

		tmpAngle += 128;
		if(tmpAngle >= 384) tmpAngle -= 384;
		SetPulse_BH((((uint32_t)DPWMMIN_48_minus7_5[tmpAngle>>3])*(PWM_Value+1))>>8);// Rapid divison by 256
	}
	Step_Num = (Angle>>6)+1;
	if(Step_Num != Old_Step){
		if(Step_Num == 1){
